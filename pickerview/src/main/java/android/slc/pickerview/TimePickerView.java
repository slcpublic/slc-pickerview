...
import android.slc.pickerview.utils.ISelectTimeCallback;
...
public class TimePickerView extends BasePickerView {
    ...
    private void initView(Context context) {
        // 时间转轮 自定义控件
        initWheelTime((LinearLayout) mPickerOptions.parentView.findViewById(R.id.timepicker));
    }
    ...
    public void returnData() {
        if (mPickerOptions.timeSelectListener != null) {
            try {
                Date date = WheelTime.dateFormat.parse(wheelTime.getTime());
                mPickerOptions.timeSelectListener.onTimeSelect(date, null);//TODO 此处第二个参数暂时设置为空
            } catch (ParseException e) {
                e.printStackTrace();
            }
        }
